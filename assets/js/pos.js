...
(function () {
    ...
    const URL_BUSCAR = window.POS_CFG.URL_BUSCAR; // "?c=caja&a=buscarProducto"
    const URL_COBRAR = window.POS_CFG.URL_COBRAR; // "?c=caja&a=cobrar"
    ...
    async function buscarYAgregar(barcode) {
      try {
        ...
        // URL_BUSCAR ya trae ?c=...&a=...
        const res = await fetch(URL_BUSCAR + '&barcode=' + encodeURIComponent(barcode), { cache: 'no-store' });
  
        // Si el backend manda HTML por error, esto truena -> catch
    ...
    async function cobrar() {
      ...
      try {
        ...
        const payload = { items: carrito.map(x => ({ id: x.id, qty: x.qty })) };
    ...
    if (btnFocus) btnFocus.addEventListener('click', focusScan);
